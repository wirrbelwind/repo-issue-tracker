...
export const useKanbanBoard = () => {
  ...
  const issuesQuery = useSearchIssues(
    repoStore.repo
      ? {
          ownerName: repoStore.repo.ownerName,
          repositoryName: repoStore.repo.name,
        }
      : null,
  );
  ...
  const onDragEndHandler = useCallback((e: DragEndEvent) => {
    ...
    if (over.data.current?.type === "Issue") {
      ...
      console.log(
        `Issue ${active.id} from board ${activeIssueBoard} to issue ${issueId} in board ${boardName}`,
      );
